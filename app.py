...
def build_page(aprendiz, resultados_sel, datos, styles):
    ...
    for i in range(num_rows):
        ...
        if entrega_info is None:
            ...
            fecha_fin = Paragraph(f_final, ParagraphStyle('ff', fontName='Helvetica', fontSize=6.5, alignment=TA_CENTER, leading=9))
        else:
...
if aprendices:
    ...
    if usar_entrega:
        ...
        for ap in aprendices:
            ...
            fecha_final = cols[2].date_input("",
                value=date.today(),
                key=f"fecha_final_{ap['nombre']}",
                label_visibility="collapsed")
