...
with col2:
    ...
    # ── BUSCADOR DIRECTO ─────────────────────────────────────────
    busqueda = st.text_input("🔍 Buscar competencia o resultado de aprendizaje",
                              placeholder="Escribe una palabra clave...",
                              key="busqueda_directa")

    if busqueda and len(busqueda) >= 3 and fases_data:
        # Buscar en todo el proyecto seleccionado
        coincidencias = []
        for fase, acts in fases_data.items():
            for act, comps in acts.items():
                for comp in comps["competencias"]:
                    # Buscar en competencia
                    if busqueda.lower() in comp["nombre"].lower():
                        coincidencias.append({
                            "tipo": "Competencia",
                            "texto": comp["nombre"],
                            "fase": fase, "act": act, "comp": comp
                        })
                    # Buscar en resultados
                    for ra in comp["resultados"]:
                        if busqueda.lower() in ra["ra"].lower():
                            coincidencias.append({
                                "tipo": "Resultado",
                                "texto": ra["ra"],
                                "fase": fase, "act": act, "comp": comp
        ...
        if coincidencias:
            st.caption(f"🎯 {len(coincidencias)} coincidencia(s) encontrada(s)")
            opciones = [f"[{c['tipo']}] {c['texto'][:80]}{'...' if len(c['texto'])>80 else ''}" 
                       for c in coincidencias]
            sel_idx = st.selectbox("Selecciona:", range(len(opciones)),
                                   format_func=lambda i: opciones[i],
                                   key="sel_busqueda")
            sel = coincidencias[sel_idx]
            fase_sel      = sel["fase"]
            actividad_sel = sel["act"]
            competencia_sel = sel["comp"]
            st.caption(f"✅ Fase: **{fase_sel}** · Actividad: **{actividad_sel[:50]}...**")
        else:
            st.caption("Sin coincidencias. Intenta con otra palabra.")

    # ── SELECCIÓN SECUENCIAL ─────────────────────────────────────
    st.markdown("<div style='color:#4caf50;font-size:.78rem;margin:.5rem 0;'>── o selecciona paso a paso ──</div>",
                unsafe_allow_html=True)
    ...
    if fase_sel != "— Selecciona —" and fase_sel in fases_data:
        ...
        actividad_sel = st.selectbox("Actividad de proyecto", ["— Selecciona —"] + acts,
                                     ...
                                     key="sel_actividad")
    elif not busqueda or len(busqueda) < 3:
        if fase_sel != "— Selecciona —" and fases_data:
            acts = list(fases_data[fase_sel].keys())
            actividad_sel = st.selectbox("Actividad de proyecto", ["— Selecciona —"] + acts,
                                         format_func=lambda x: (x[:70]+"...") if len(x)>70 else x,
                                         key="sel_actividad2")

    if actividad_sel != "— Selecciona —" and not competencia_sel:
