...
def leer_aprendices(file):
    import io, subprocess, tempfile, os, re as _re
    import pandas as pd
    ...
    if nombre_archivo.lower().endswith('.xls'):
        ...
        subprocess.run(['libreoffice','--headless','--convert-to','xlsx','--outdir',out_dir, tmp_path],
                       capture_output=True)
    ...
    # Buscar fila de encabezados — la que tenga "Nombre" o "NOMBRE"
    header_row = None
    col_idx = {}
    for i, row in enumerate(ws.iter_rows(values_only=True)):
        row_lower = [str(c).strip().lower() if c else '' for c in row]
        if 'nombre' in row_lower:
            header_row = i
            for j, val in enumerate(row_lower):
                if 'nombre' in val and 'apellido' not in val:   col_idx['nombre'] = j
                if 'apellido' in val:                            col_idx['apellido'] = j
                if 'tipo' in val and 'doc' in val:               col_idx['tipo_doc'] = j
                if ('número' in val or 'numero' in val or 'n°' in val) and 'doc' in val: col_idx['num_doc'] = j
                if 'documento' in val and 'tipo' not in val:     col_idx['num_doc'] = j
                if 'estado' in val:                              col_idx['estado'] = j
            break

    if header_row is None:
        # Fallback: asumir estructura fija (col 0=tipo, 1=num, 2=nombre, 3=apellidos, 4=estado)
        col_idx = {'tipo_doc':0,'num_doc':1,'nombre':2,'apellido':3,'estado':4}
        header_row = 0

    aprendices = []
    vistos = set()  # Para deduplicar por número de documento

    tiene_estado = 'estado' in col_idx

    for row in ws.iter_rows(min_row=header_row+2, values_only=True):
        try:
            nombre = str(row[col_idx.get('nombre', 2)] or '').strip()
            if not nombre or nombre.lower() in ('nombre','nan',''): continue

            apellido = str(row[col_idx.get('apellido', 3)] or '').strip()
            nombre_completo = f"{nombre} {apellido}".strip()

            tipo_doc = str(row[col_idx.get('tipo_doc', 0)] or 'CC').strip()
            num_doc  = str(row[col_idx.get('num_doc',  1)] or '').strip()

            # Filtrar por estado si existe la columna
            if tiene_estado:
                estado = str(row[col_idx.get('estado', 4)] or '').strip().upper()
...
with col1:
    ...
    ficha_raw = st.text_input("Número de ficha", placeholder="Ej. 2441890")
    # Solo números
