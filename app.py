...
logo_html = (f'<img src="data:image/png;base64,{logo_b64}" '
            ...
            '<span style="color:white;font-weight:900;font-size:1.4rem;background:white;color:#006633;padding:6px 10px;border-radius:6px;">SENA</span>'

st.markdown(f"""
<style>
.main-header{{background:linear-gradient(135deg,#006633 0%,#009944 100%);padding:1.2rem 2rem;border-radius:12px;margin-bottom:1.5rem;display:flex;align-items:center;gap:1.5rem}}
.main-header-text h1{{color:white;margin:0;font-size:1.5rem;line-height:1.2}}
.main-header-text p{{color:#ccffcc;margin:4px 0 0;font-size:.85rem}}
.section-box{{background:#f8fffe;border:1px solid #c8e6c9;border-radius:10px;padding:1.2rem 1.5rem;margin-bottom:1rem}}
.section-title{{color:#006633;font-weight:700;font-size:.85rem;margin-bottom:.8rem;text-transform:uppercase;letter-spacing:.05em}}
...
<div class="main-header">
  ...
  <div class="main-header-text">
    ...
    <p>Servicio Nacional de Aprendizaje SENA · Centro Nacional Colombo Alemán · Cód. 821100 v1</p>
  </div>
</div>
...
CATALOGO = {
  ...
  "821100 v1 — Producción de Componentes Mecánicos con Máquinas CNC": {
    "proyectos": {
      ...
      "2441890 — Fabricar elementos mecánicos con tecnología CNC": {
        "nombre_completo": "FABRICAR ELEMENTOS MECÁNICOS CON TECNOLOGÍA DE CONTROL NUMÉRICO COMPUTARIZADO",
        "fases": {
          "INDUCCIÓN": {
      ...
      "3343700 — Fabricación de componentes mecánicos con máquinas herramientas CNC": {
        "nombre_completo": "FABRICACIÓN DE COMPONENTES MECÁNICOS CON MÁQUINAS HERRAMIENTAS CNC PARA EL SECTOR INDUSTRIAL DEL ATLÁNTICO",
        "fases": {
          "IDENTIFICACIÓN": {
...
def build_page(aprendiz, resultados_sel, datos, styles):
    ...
    # Fecha actual para columna Concertada
    fecha_hoy = date.today().strftime("%d/%m/%Y")

    # Logo en PDF
    logo_img = get_logo_image()
    logo_cell = logo_img if logo_img else Paragraph(
        "<b>SENA</b>", ParagraphStyle('lg', fontName='Helvetica-Bold', fontSize=16,
                                       alignment=TA_CENTER, textColor=SENA_GREEN))

    # Ancho total de página (letter - márgenes = 17.5 cm)
    PAGE_W = 17.5*cm

    header = Table([[
        logo_cell,
        Paragraph("<b>SERVICIO NACIONAL DE APRENDIZAJE SENA</b><br/><b>CENTRO NACIONAL COLOMBO ALEMAN</b><br/>PLAN CONCERTADO",
    ...
    # Ancho de columnas — igual para título y tabla de descriptores
    col_w = [5*cm, 1.2*cm, 5.5*cm, 1.1*cm, 1.1*cm, 1.5*cm, 1.5*cm, 1.3*cm, 1.3*cm]
    total_w = sum(col_w)  # = 19.5 cm... ajustamos para que cuadren

    h = styles['header']
    num_rows = max(len(actividades_todas), 5)

    # Título descriptores con mismo ancho que la tabla
    desc_title = Table([[
    ...
    for i in range(num_rows):
        ...
        table_data.append([
            ...
            Paragraph("X", styles['cell_c']),   # ← X en Física
            '',                                   # Digital vacío
            Paragraph(fecha_hoy, styles['cell_c']),  # ← fecha en Concertada
            '',                                   # Final vacío
            '', ''                                # SI / NO vacíos
        ])
    ...
    desc_table.setStyle(TableStyle([
        ...
        # X en negrita y verde
        ('TEXTCOLOR',(3,2),( 3,1+num_rows), SENA_GREEN),
        ('FONTNAME',(3,2),(3,1+num_rows),'Helvetica-Bold'),
        ('FONTSIZE',(3,2),(3,1+num_rows),10),
    ]))
...
with col_btn:
    generar = st.button("🖨️ Generar Plan Concertado PDF")
...
if generar:
    ...
    if errores:
        ...
        st.success(f"✅ Plan Concertado generado — {len(aprendices)} páginas")
        st.download_button(
            label=f"⬇️ Descargar Plan_Concertado_Ficha_{ficha}.pdf",
            data=pdf_bytes,
            file_name=f"Plan_Concertado_Ficha_{ficha}.pdf",
